...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.propostas ADD COLUMN usuario_id UUID REFERENCES auth.users(id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'clientes' AND column_name = 'ativo'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'bancos' AND column_name = 'ativo'
  ) THEN
    ALTER TABLE public.bancos ADD COLUMN ativo BOOLEAN DEFAULT true;
  END IF;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'proposta_historico') THEN
    ALTER TABLE public.proposta_historico ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view historico from their empresa" ON public.proposta_historico;
CREATE POLICY "Users can view historico from their empresa"
ON public.proposta_historico FOR SELECT
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can create historico in their empresa" ON public.proposta_historico;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'proposta_anexos') THEN
    ALTER TABLE public.proposta_anexos ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view anexos from their empresa" ON public.proposta_anexos;
CREATE POLICY "Users can view anexos from their empresa"
ON public.proposta_anexos FOR SELECT
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can create anexos in their empresa" ON public.proposta_anexos;
CREATE POLICY "Users can create anexos in their empresa"
ON public.proposta_anexos FOR INSERT
WITH CHECK (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can delete anexos in their empresa" ON public.proposta_anexos;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'proposta_atividades') THEN
    ALTER TABLE public.proposta_atividades ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view atividades from their empresa" ON public.proposta_atividades;
CREATE POLICY "Users can view atividades from their empresa"
ON public.proposta_atividades FOR SELECT
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can create atividades in their empresa" ON public.proposta_atividades;
CREATE POLICY "Users can create atividades in their empresa"
ON public.proposta_atividades FOR INSERT
WITH CHECK (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can update atividades in their empresa" ON public.proposta_atividades;
CREATE POLICY "Users can update atividades in their empresa"
ON public.proposta_atividades FOR UPDATE
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can delete atividades in their empresa" ON public.proposta_atividades;
...
CREATE TABLE IF NOT EXISTS public.proposta_documentos (
  ...
  status_documento TEXT DEFAULT 'pendente',
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'proposta_documentos') THEN
    ALTER TABLE public.proposta_documentos ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view documentos from their empresa" ON public.proposta_documentos;
CREATE POLICY "Users can view documentos from their empresa"
ON public.proposta_documentos FOR SELECT
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can create documentos in their empresa" ON public.proposta_documentos;
CREATE POLICY "Users can create documentos in their empresa"
ON public.proposta_documentos FOR INSERT
WITH CHECK (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can update documentos in their empresa" ON public.proposta_documentos;
CREATE POLICY "Users can update documentos in their empresa"
ON public.proposta_documentos FOR UPDATE
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can delete documentos in their empresa" ON public.proposta_documentos;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'contratos_apolices') THEN
    ALTER TABLE public.contratos_apolices ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view contratos from their empresa" ON public.contratos_apolices;
CREATE POLICY "Users can view contratos from their empresa"
ON public.contratos_apolices FOR SELECT
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can create contratos in their empresa" ON public.contratos_apolices;
CREATE POLICY "Users can create contratos in their empresa"
ON public.contratos_apolices FOR INSERT
WITH CHECK (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can update contratos in their empresa" ON public.contratos_apolices;
CREATE POLICY "Users can update contratos in their empresa"
ON public.contratos_apolices FOR UPDATE
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Admins can delete contratos in their empresa" ON public.contratos_apolices;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'comissoes') THEN
    ALTER TABLE public.comissoes ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view comissoes from their empresa" ON public.comissoes;
CREATE POLICY "Users can view comissoes from their empresa"
ON public.comissoes FOR SELECT
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can create comissoes in their empresa" ON public.comissoes;
CREATE POLICY "Users can create comissoes in their empresa"
ON public.comissoes FOR INSERT
WITH CHECK (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Users can update comissoes in their empresa" ON public.comissoes;
CREATE POLICY "Users can update comissoes in their empresa"
ON public.comissoes FOR UPDATE
USING (empresa_id = get_user_empresa_id(auth.uid()));

DROP POLICY IF EXISTS "Admins can delete comissoes in their empresa" ON public.comissoes;
